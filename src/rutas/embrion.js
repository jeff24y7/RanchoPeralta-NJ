...
// DELETE EN COMPRAS EMBRION
router.delete('/deleteembrion/:COD_COMPRA_EMBRION', (req, res) => {
  const { COD_COMPRA_EMBRION } = req.params;
console.log(COD_COMPRA_EMBRION)
  mysqlConnection.query('call DLT_COMPRAEM(?)', [COD_COMPRA_EMBRION], (err, rows, fields) => {
    if(!err) {
      res.json({status: 'ELIMINADO DE HISTORIAL DE EMBRION'});
    } else {
      console.log(err);
    }
  });
...
router.get('/transferencias-de-embriones', (req,res)=>{
    mysqlConnection.query('SELECT * FROM MG_TRANS_EMBRION', (err,rows,fields)=>{
        if(!err){
            res.json(rows);
        }else{
            console.log(err);
